...
def get_averages(image, grid_sizes=grid_sizes, MODEL=MODEL):
  image_r = cv2.imread(image)
  predictions = []
  for size in grid_sizes:
    subimages = get_subimages(image_r, size)
    grid_predictions = []
  ...
  avg_item_scores = {item: sum(scores)/len(scores) for item, scores in item_scores.items()}
  sorted_items = sorted(item_scores.keys(), key=lambda x: avg_item_scores[x], reverse=True)
  sorted_item_scores = [avg_item_scores[item] for item in sorted_items]
  ret = (sorted_items, sorted_item_scores)
  print(ret)
  return ret
...
def get_mickey(image):
  results = MODEL(image)
  top_foods, top_prob = get_prob(results)
  ret = top_foods, top_prob
  print(ret)
  return ret
